...
namespace Factory
{
    class OracleHelper : IDataBase
    {
        //Server查询数据
        public List<T> Query<T>(string Server, string strSql, DynamicParameters Parameters)
        {
            using (IDbConnection conn = new OracleConnection(Server))
            {
        ...
        {
            //Server写 获取一个数据库连接
            using (IDbConnection connection = new OracleConnection(Server))
            {
                connection.Open();
        ...
        {
            using (IDbConnection dbConnection = new OracleConnection(Server))
            {
                dbConnection.Open();
        ...
        {
            using (IDbConnection con = new OracleConnection(Server))
            {
